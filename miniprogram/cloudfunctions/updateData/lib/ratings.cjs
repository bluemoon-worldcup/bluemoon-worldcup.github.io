...
async function fetchRatings(fetchJson, matches, log) {
  ...
  const byDate = {}
  targets.forEach(m => {
    const d = m.utc.slice(0, 10).replace(/-/g, '')
  ...
  for (const date of Object.keys(byDate)) {
    let list
    try { list = await fetchJson(FM_MATCHES + date) } catch (e) { log('FotMob 列表失败 ' + date + ': ' + e.message); continue }
    const fms = (list && list.leagues ? list.leagues : []).reduce((a, l) => a.concat(l.matches || []), [])
